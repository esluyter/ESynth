  ...
  *new { |parent, bounds|
  ...
  init { |parent, bounds|
    ...
    7.do { |i|
      ...
      StaticText(view, Rect(4 + (30 * i) + (if (i < 4) { 0 } { spacer }), 55, 25, 10)).string_([nil, "cutoff", "res", "mod", "a", "d", "s", "r"][i]).font_(Font.sansSerif.size_(8)).align_(\center);
    };
    4.do { |i|
      i = i + 1;
      Knob(view, Rect(4 + (30 * i) + (if (i < 4) { 0 } { spacer }), 65, 25, 25));
      NumberBox(view, Rect(4 + (30 * i) + (if (i < 4) { 0 } { spacer }), 89, 25, 10)).font_(Font.monospace.size_(8)).align_(\center);
      StaticText(view, Rect(4 + (30 * i) + (if (i < 4) { 0 } { spacer }), 100, 25, 10)).string_([nil, "key", "vel", "env", "delay"][i]).font_(Font.sansSerif.size_(8)).align_(\center);
    };
  }
